...
class MacroIndexBot:
    ...
    async def _handle_direction_change(self, score):
        """
        AUTOMATIC MACRO FLIPPING - Closes old positions and opens new ones
        - When macro flips from LONG → SHORT (or vice versa), closes all positions and opens new ones
        - When macro goes to FLAT, closes all positions
        - When macro signals LONG/SHORT from FLAT, opens positions
        """
        old_direction = self.current_direction
        new_direction = score.direction

        # Case 1: Going from FLAT to a direction - just open positions
        ...
        # Case 2: Direction flip (LONG → SHORT or SHORT → LONG) - close old, open new
        elif old_direction != MacroDirection.FLAT and new_direction != MacroDirection.FLAT and old_direction != new_direction:
            logger.info(f"{'='*60}")
            logger.info(f"🔄 MACRO FLIP: {old_direction.value} → {new_direction.value}")
            logger.info(f"Closing all {old_direction.value} positions and opening {new_direction.value}")
            logger.info(f"{'='*60}")

            # Close all old positions
            await self._close_all_positions_for_direction(old_direction.value)

            # Open new positions in new direction
            await self._open_all_positions(new_direction.value)
            self.current_direction = new_direction

        # Case 3: Going to FLAT - close all positions
        elif old_direction != MacroDirection.FLAT and new_direction == MacroDirection.FLAT:
            logger.info(f"{'='*60}")
            logger.info(f"📉 MACRO FLAT: Closing all {old_direction.value} positions")
            logger.info(f"{'='*60}")
            await self._close_all_positions_for_direction(old_direction.value)
            self.current_direction = new_direction
